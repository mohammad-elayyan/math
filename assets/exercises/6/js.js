...
window.onload = () => {
  ...
  let points = [
    ...
    [getProjectedX(-1), getProjectedY(-2), 3],
    [getProjectedX(-1), getProjectedY(0), 3],
  ];
  let slop_arr = [];
  let c_arr = [];
  let distance_array = [];
  ...
  let opt_selector = "draw-shape";
   ...
   write_txt("إبدأ", getProjectedX(0), getProjectedY(10),'4vw tj-b');
...
function check_drawings(tempShape,tempPoints)
{
  let tolerance=0.1;
    ...
    if (tempPoints.length != tempShape.length) 
        return false;
